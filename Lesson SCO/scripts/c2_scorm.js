...
var strLessonStatus = "incomplete"; //a str holding the lesson satus: 2--completed; 1--in process; 0--not started yet
...
function startCourse() {
	...
	if (inLMS == true) {
		...
		if ( strTemp == "301" ) {	//NotInitialized
			...
			strLessonStatus = "incomplete" //arrTemp[0];		// get lesson status in 1st group 
			strPagesViewed = arrTemp;		// get page reviewed in current lesson in 2nd group
			}
		}
	...
	startPage = "lesson"+lID +"/1"+lID+ "_010.htm"; //set a main menu
	if ( bookmark != "" ) {
...
//			else if (lesson == 2) nPages = 20  ////change this number based on the pages in lesson 2
			...
			contentFrame.location.href = bookmark; // go to bookmarked page
...
function exitCourse() {
	...
	if ( exitPageStatus != true ) {
		...
		if ( inLMS == true ) {
			if (contentFrame.blnLastPage) doLMSSetValue( "cmi.core.lesson_status", "completed" );
...
function gotoPage(direction, pgURL) {
	...
	if (isPageViewed(getPage()) == false && direction == "f")  {
		  if (contentFrame.blnLastPage)  exitCourse() //premenu();
...
function saveBookmark() {
	...
	var strBookmark = "";
	if (contentFrame.blnLastPage!= true )  {
...
function updateSuspendData() {
	...
	strTemp = strPagesViewed; //strLessonStatus + "~" + strPagesViewed;
	if (inLMS == true) {
